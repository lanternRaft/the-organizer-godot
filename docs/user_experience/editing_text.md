...
**Enter** (without Shift) closes the editor and saves the text. The text appears on the shape permanently, rendered in a crisp white font. The canvas auto-saves, so the text persists across sessions.
...
Selecting a shape is the first step to editing its text — see [Selecting Elements](selecting_and_manipulating/selecting_elements.md) for how selection works.